...
class TransformerConfig(ModelParallelConfig):
    ...
    gengram_enabled: bool = False
    """Enable Gengram n-gram retrieval residual in selected layers."""

    gengram_layer_ids: Optional[List[int]] = None
    """1-based global transformer layer numbers to insert Gengram into.
    Example: [1, 15] inserts into layer 1 and 15.
    If None/empty and enabled, will raise in __post_init__."""

    gengram_ngram_sizes: Tuple[int, ...] = (2, 3)
    """Which n-grams to retrieve. For DNA this is typically (2, 3).
    Each n uses a table of size 5**n."""

    gengram_embed_dim_per_ngram: int = 128
    """Embedding width for each n-gram table (per n).
    Total retrieved feature width = len(ngram_sizes) * embed_dim_per_ngram."""

    gengram_token_ids: Optional[Tuple[int, int, int, int, int]] = (8, 5, 6, 7, 9)
    """Token ids for the 5 DNA letters (A,C,G,T,N) in your tokenizer.
    Default matches the mapping the user provided: A=8,C=5,G=6,T=7,N=9.

    IMPORTANT: if your tokenizer uses different ids, you MUST override this."""

    gengram_use_short_conv: bool = True
    """Whether to apply a depthwise short convolution over the Engram residual."""

    gengram_short_conv_kernel_size: int = 4
    """Kernel size for the depthwise short convolution."""

    gengram_short_conv_dilation: Optional[int] = None
    """Dilation for the short convolution. If None, defaults to max(gengram_ngram_sizes)."""

    gengram_window_size: int = 1024
    """Window size W (in tokens) for Gengram windowed pooling.

    For each position t, Gengram pools all n-grams of size n that are fully contained
    in the causal token window [t-W+1, t]. This corresponds to pooling n-gram *ending* positions
    in the last (W-n+1) steps.

    Must satisfy: W >= max(gengram_ngram_sizes)."""
    ...
    def __post_init__(self):
        ...
        # Gengram validation / defaults
        if self.gengram_enabled:
            # If layer ids is not provided, we interpret it as "all layers".
            # (This also keeps dist checkpoint structure consistent.)
            if not self.gengram_layer_ids:
                self.gengram_layer_ids = []
            else:
                # Normalize layer ids to ints and validate range
                self.gengram_layer_ids = [int(x) for x in self.gengram_layer_ids]
                for lid in self.gengram_layer_ids:
                    if lid < 1 or lid > self.num_layers:
                        raise ValueError(
                            f'gengram_layer_ids contains {lid}, but valid range is [1, {self.num_layers}]'
                        )

            if self.gengram_token_ids is None or len(self.gengram_token_ids) != 5:
                raise ValueError(
                    'gengram_enabled=True requires gengram_token_ids with exactly 5 ids (A,C,G,T,N).'
                )

            if any(int(n) < 1 for n in self.gengram_ngram_sizes):
                raise ValueError(f'gengram_ngram_sizes must all be >=1, got {self.gengram_ngram_sizes}')

            # Window size W for windowed n-gram pooling.
            # Must be >= max(ngram_sizes) so that each n has at least one ending position (W-n+1 >= 1).
            self.gengram_window_size = int(self.gengram_window_size)
            if self.gengram_window_size < 1:
                raise ValueError(f'gengram_window_size must be >=1, got {self.gengram_window_size}')
            _max_n = int(max(self.gengram_ngram_sizes))
            if self.gengram_window_size < _max_n:
                raise ValueError(
                    f'gengram_window_size ({self.gengram_window_size}) must be >= max(gengram_ngram_sizes) ({_max_n})'
                )

            if self.gengram_short_conv_dilation is None:
                self.gengram_short_conv_dilation = int(max(self.gengram_ngram_sizes))
