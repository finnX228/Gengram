...
class TransformerLayer(GraphableMegatronModule, BaseTransformerLayer):
    ...
    def __init__(
        ...
        # [Optional: Gengram n-gram retrieval residual]
        self.gengram = None
        self.gengram_active = False
        if getattr(self.config, 'gengram_enabled', False):
            from megatron.core.transformer.gengram import Gengram

            layer_ids = getattr(self.config, 'gengram_layer_ids', None)
            if layer_ids is None or len(layer_ids) == 0:
                active = True
            else:
                active = self.layer_number in layer_ids

            self.gengram_active = bool(active)
            self.gengram = Gengram(
                hidden_size=self.config.hidden_size,
                dna_token_ids=self.config.gengram_token_ids,
                ngram_sizes=self.config.gengram_ngram_sizes,
                embed_dim_per_ngram=self.config.gengram_embed_dim_per_ngram,
                active=active,
                use_short_conv=self.config.gengram_use_short_conv,
                short_conv_kernel_size=self.config.gengram_short_conv_kernel_size,
                short_conv_dilation=self.config.gengram_short_conv_dilation,
                window_size=self.config.gengram_window_size,
            )
    ...
    def _forward_attention(
        ...
        *,
        ...
        # Gengram residual (active layers only)
        if self.gengram is not None and self.gengram_active:
            if input_ids is None:
                raise ValueError(
                    'Gengram is enabled but input_ids was not provided. '
                    'Pass input_ids through TransformerBlock/GPTModel.'
                )
            hidden_states = hidden_states + self.gengram(hidden_states, input_ids)
