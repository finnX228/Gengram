...
def add_megatron_arguments(parser: argparse.ArgumentParser):
    ...
    parser = _add_gengram_args(parser)
...
def _add_gengram_args(parser):
    group = parser.add_argument_group(title="gengram")

    group.add_argument("--gengram-enabled", action="store_true",
                       help="Enable Gengram n-gram retrieval residual.")
    group.add_argument("--gengram-layer-ids", type=int, nargs="+", default=None,
                       help="Layer ids (1-based) to enable Gengram, e.g. 1 15.")
    group.add_argument("--gengram-ngram-sizes", type=int, nargs="+", default=[2, 3],
                       help="N-gram sizes to use, e.g. 2 3.")
    group.add_argument("--gengram-window-size", type=int, default=1024,
                       help="Window size W (tokens) for Gengram windowed pooling. Must be >= max(ngram_sizes).")
    group.add_argument("--gengram-embed-dim-per-ngram", type=int, default=128,
                       help="Embedding dim per n-gram table (concat then projected to hidden).")
    group.add_argument("--gengram-token-ids", type=int, nargs=5, default=[8, 5, 6, 7, 9],
                       help="Exactly 5 token ids for [A,C,G,T,N] (or your chosen order).")
    group.add_argument("--gengram-use-short-conv", action="store_true",
                       help="Enable depthwise short conv over retrieved values.")
    group.add_argument("--gengram-short-conv-kernel-size", type=int, default=4,
                       help="Kernel size for short conv.")
    group.add_argument("--gengram-short-conv-dilation", type=int, default=None,
                       help="Dilation for short conv. Default = max(ngram_sizes).")
    return parser
