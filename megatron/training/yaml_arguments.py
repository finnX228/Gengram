...
def core_config_from_args(args, dataclass=TransformerConfig):
    """Builds core config dict from a namespace args for a dataclass.

    For YAML-config flows, older YAML files may omit newly added fields.
    To preserve backward-compatibility, missing fields fall back to dataclass defaults
    ...
    for f in dataclasses.fields(dataclass):
        ...
        # Backward-compatibility: fall back to dataclass defaults if present.
        if f.default is not dataclasses.MISSING:
            kw_args[f.name] = f.default
            continue
        if getattr(f, 'default_factory', dataclasses.MISSING) is not dataclasses.MISSING:  # type: ignore[attr-defined]
            kw_args[f.name] = f.default_factory()  # type: ignore[misc]
            continue
